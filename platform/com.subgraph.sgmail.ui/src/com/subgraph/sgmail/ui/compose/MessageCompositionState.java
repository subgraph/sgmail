...
public class MessageCompositionState {
    ...
    MessageCompositionState(IEventBus eventBus, MessageProcessor messageProcessor, IdentityManager identityManager, MessageComposer composer, StoredMessage replyMessage) {
    ...
    public MimeMessage createMessage(String bodyText) throws MessagingException {
        ...
        if(isSigningRequested || isEncryptRequested) {
            OpenPGPProcessing openpgp = new OpenPGPProcessing(messageProcessor, identityManager, selectedAccount, msg,  isEncryptRequested, isSigningRequested);
            if(openpgp.process()) {
    ...
    private void addToContactList(List<Contact> contacts, InternetAddress address) {
        final Contact c = identityManager.getContactByEmailAddress(address.getAddress());
        contacts.add(c);
        /*
