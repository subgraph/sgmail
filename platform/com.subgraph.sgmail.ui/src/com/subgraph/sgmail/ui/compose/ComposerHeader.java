...
public class ComposerHeader extends Composite {
  ...
  ComposerHeader(Composite parent, MessageCompositionState state, Model model,
  ...
  public void populateForReply(StoredMessage message, boolean isReplyAll) {
    final RecipientSection toSection = recipientSectionMap
        .get(RecipientType.TO);
  ...
  private String getReplyTo(StoredMessage message) throws MessagingException {
    ...
    final Address[] replyTo = message.toMimeMessage(
        javamailUtils.getSessionInstance()).getReplyTo();
  ...
  private static String getReplySubject(StoredMessage message)
  ...
  private void addRecipientSection(RecipientType type,
      ModifyListener modifyListener) {
    recipientSectionMap.put(type, new RecipientSection(state, this, type,
        modifyListener));
  }
