...
public class StoredMessageSummary implements Activatable {
  ...
  StoredMessageSummary(StoredMessageBuilder builder, StoredMessageContent content) {
    this(builder.subject.getBytes(Charsets.UTF_8), builder.bodySnippet.getBytes(Charsets.UTF_8), builder.sender,
        content);
  }
  ...
  void setDecryptedMessageDetails(byte[] decryptedRawBytes, String decryptedBody,
      List<MessageAttachment> decryptedAttachments) {
  ...
  void setSignatureStatus(int value) {
    activate(ActivationPurpose.WRITE);
    if(value < 0 || value > StoredMessage.SIGNATURE_NO_PUBKEY) {
