...
public class Server {
	...
	public Server(IRandom random, Properties properties) {
    ...
    public KeyRegistrationState createRegistrationState(String emailAddress, PGPPublicKeyRing publicKeyRing) {
        synchronized (keyRegistrationLock) {
            ...
            final SecureRandom sr = random.getSecureRandom();
            final KeyRegistrationState krs = new KeyRegistrationState(emailAddress, publicKeyRing, sr.nextLong(), sr.nextLong(), new Date());
	...
	public static void main(String[] args) {
        if(args.length != 1) {
            ...
            try {
            	// XXX
                final Server server = new Server(null, loadProperties(args[0]));
                server.start();
            } catch (IOException e) {
                e.printStackTrace();
            }
