...
public class StoredIMAPMessageFactory {
  ...
  public StoredIMAPMessageFactory(Model model, JavamailUtils javamailUtils,
      MessageFactory basicMessageFactory) {
    this.model = model;
    this.javamailUtils = javamailUtils;
    this.basicMessageFactory = basicMessageFactory;
  }

  public StoredMessage createFromJavamailMessage(IMAPAccount imapAccount,
      MimeMessage message) throws MessagingException, IOException {
    final StoredMessage duplicate = imapAccount
        .getMessageForMimeMessage(message);
    if (duplicate != null) {
      return duplicate;
    }
    final int conversationId = imapAccount
        .generateConversationIdForMessage(message);
    final int messageId = imapAccount.generateUniqueMessageIdForMessage(
        message, model);
    final List<StoredMessageLabel> gmailLabels = getGmailLabels(imapAccount,
        message);

    final MimeMessage reparsed = reparseMessage(message);
    final StoredMessage sm = createFromJavamailMessage(reparsed, messageId,
        conversationId, gmailLabels);

    addEncryptionFlags(getMultipartType(reparsed), sm);
  ...
  private void addEncryptionFlags(String multipartType, StoredMessage msg) {
    boolean encrypted = false;
    boolean signed = false;

    if(multipartType.equalsIgnoreCase("encrypted")) {
      encrypted = true;
    } else if(multipartType.equalsIgnoreCase("signed")) {
      signed = true;
    } else if(msg.getBodyText().contains(BEGIN_PGP)) {
      if(msg.getBodyText().contains(BEGIN_PGP_ENCRYPTED)) {
        encrypted = true;
      } else if(msg.getBodyText().contains(BEGIN_PGP_SIGNED)) {
        signed = true;
      }
    }
    
    if(encrypted) {
      msg.addFlag(StoredMessage.FLAG_ENCRYPTED);
    } else if(signed) {
      msg.addFlag(StoredMessage.FLAG_SIGNED);
      msg.setSignatureStatus(StoredMessage.SIGNATURE_UNVERIFIED);
    }
  }
  ...
  public StoredMessage createFromJavamailMessage(MimeMessage message,
      MessageIdGenerator idGenerator) throws MessagingException, IOException {
  ...
  private StoredMessage createFromJavamailMessage(MimeMessage message,
      int messageId, int conversationId, List<StoredMessageLabel> labels)
      throws MessagingException, IOException {
    return basicMessageFactory
        .createStoredMessageBuilder(readRawBytes(message))
        ...
        .flags(getMessageFlags(message)).labels(labels)
        .attachments(javamailUtils.getAttachments(message)).build();
  }

  private MimeMessage reparseMessage(MimeMessage message) throws IOException,
      MessagingException {
  ...
  private List<MessageUser> getRecipients(MimeMessage message,
  ...
  private void addRecipientsToList(List<MessageUser> recipientList,
      MimeMessage message, Message.RecipientType recipientType)
  ...
  private long getMessageTimestamp(MimeMessage message)
      throws MessagingException {
  ...
  private List<StoredMessageLabel> getGmailLabels(IMAPAccount account,
    ...
    if (gmailMessage.getLabels() == null
        || gmailMessage.getLabels().length == 0) {
      return null;
    }
